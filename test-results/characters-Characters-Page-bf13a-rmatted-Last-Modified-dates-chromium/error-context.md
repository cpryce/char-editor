...
- Name: characters.spec.ts >> Characters Page >> renders formatted Last Modified dates
- Location: e2e/characters.spec.ts:45:7
...
Locator: getByRole('row').nth(1).getByRole('cell').nth(3)
Expected: not empty
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('row').nth(1).getByRole('cell').nth(3)
