...
- Name: character-editor.spec.ts >> Character Editor >> Form Submission >> submitting a valid form POSTs to /api/characters and navigates back
- Location: e2e/character-editor.spec.ts:137:9
...
Call log:
  - waiting for locator('select').filter({ hasText: '— Select class —' })
...
> 13  |   await page.locator('select').filter({ hasText: '— Select class —' }).selectOption(className);
  ...
  109 | 
  110 |       await expect(page.getByRole('button', { name: 'Save Character' })).toBeDisabled();
  111 |     });
  112 | 
  113 |     test('Save Character is disabled when name is filled but no class selected', async ({ page }) => {
```