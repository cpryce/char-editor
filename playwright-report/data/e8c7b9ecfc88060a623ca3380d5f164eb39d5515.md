...
- Name: characters.spec.ts >> Characters Page >> Custom Feats sidebar item is disabled
- Location: e2e/characters.spec.ts:81:7
...
Locator:  getByRole('button', { name: 'Custom Feats' })
Expected: disabled
Received: enabled
Timeout:  5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: 'Custom Feats' })
    9 × locator resolved to <button class="w-full text-left py-1.5 text-xs font-semibold uppercase tracking-wider pl-6 pr-3 bg-transparent text-[color:var(--color-fg-muted)] cursor-pointer not-italic">Custom Feats</button>
